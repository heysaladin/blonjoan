...
class ItemLineDetailsAdapter(private val mContext: Context, private val mFlowerList: List < ItemsModel > ? ): RecyclerView.Adapter < ItemLineDetailsViewHolder > () {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ItemLineDetailsViewHolder {
        val mView = LayoutInflater.from(parent.context).inflate(R.layout.recyclerview_row_item_inline_details, parent, false)
        return ItemLineDetailsViewHolder(mView)
    }

    override fun onBindViewHolder(holder: ItemLineDetailsViewHolder, position: Int) {
...
class ItemLineDetailsViewHolder(itemView: View): RecyclerView.ViewHolder(itemView) {
